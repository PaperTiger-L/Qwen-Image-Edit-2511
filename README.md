...
- 批量任务提交后会在后台执行，当前页面可查看总进度；刷新页面后，若当前浏览器记录的最近一次批量任务仍在 7 天有效期内，则会自动恢复其状态。
...
- 两种批量模式都会在后台执行，页面会轮询显示总进度与当前状态；刷新页面后，若当前浏览器记录的最近一次批量任务仍在 7 天有效期内，则会自动恢复运行中、已完成或已失败状态。
- 若该任务已成功完成且结果 ZIP 仍存在，则刷新后会继续显示下载入口；若任务已过期或已被清理，则页面不再恢复历史任务状态。
